...
const TodoEdit = () => {
  ...
  const { selectedTodoInfo, todoEditFlag } = useSelector((state: TotalInitialstate) => state.todo);
  ...
  const statusList = [
    { dtlCd: "todo", dtlNm: "Todo" },
    { dtlCd: "doing", dtlNm: "Doing" },
    { dtlCd: "done", dtlNm: "Done" },
  ];
  // edit인지 update인지 체크
  useEffect(() => {
    if (todoEditFlag && selectedTodoInfo.todo_id!==-1) {
      setUpdateFlag(true);
      setFormData(selectedTodoInfo);
    }
  }, [selectedTodoInfo, todoEditFlag]);
  ...
  return (
    <form className="edit-form">
      <ul className="edit-button-wrap">
        <button type="button" className="edit-button" onClick={handleSaveFormData}>
          Save
        </button>
        <button type="button" className="edit-button jh-mx-xs" onClick={handleResetFormData}>
          Reset
        </button>
