...
import { deleteSelectedTodoInfo, setSelectedTodoInfo, toggleEditFlag } from "../../stores/actions/todo";
...
const TodoItem = ({ info, idx }) => {
  ...
  const handleDeleteItem = useCallback(
    (id: number) => {
      dispatch(deleteSelectedTodoInfo(id));
    },
    [dispatch]
  );
  ...
  const handleUpdateItem = useCallback(() => {
    ...
    dispatch(setSelectedTodoInfo(info));
  }, [dispatch, info]);
  ...
  return (
    <li key={info.todo_id} className="todo-list-item">
      ...
      <span className="todo-info-range-date">
        {info.start} ~ {info.end}
      </span>
